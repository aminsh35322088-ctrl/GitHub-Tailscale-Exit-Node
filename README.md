...
Repository access را فقط به Fork خودتان محدود کنید و حداقل این Permission را بدهید:
...
برای شروع دوباره، `EXIT_NODE_DISABLED` را روی `false` قرار دهید یا حذف کنید و سپس `action=ensure` را اجرا کنید.
